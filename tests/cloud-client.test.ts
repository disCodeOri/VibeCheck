...
describe('cloud image serialization',()=>{
 ...
 it('preserves all state and maps every stored image including saved previews',async()=>{
  ...
  expect(result.profile.photo).toBe('mapped:face');expect(result.profile.bodyPhoto).toBe('mapped:body');expect(result.garments[0].image).toBe('mapped:/assets/garments-soft.png#0');expect(result.saved[0].secondaryImage).toBe('mapped:preview');expect(result.saved[0].title).toBe(original.saved[0].title);expect(original.profile.photo).toBe('face');expect(result.saved[0].analysis).toEqual(exampleAnalysis);
 });
});
